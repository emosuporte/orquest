...
async function obterRespostasDeTodasAPIs(constraint, values, baseNacionalData) {
    const respostas = {};
    const acessos = getUserAccess();
    const carData = await getData({ method: 'GET', url: 'https://cardadosv2.bubbleapps.io/version-test/api/1.1/obj/cadastro_api' });


    for (const acesso in acessos) {
        try {
            let didRequest = false;
            const carDataFiltered = filterData(carData.data.response.results, 'ambito', acesso);
            let requestCount = carDataFiltered.length;
            let carDataTypes = [];
            ...
            for (let i of carDataTypes) {
                ...
                for (let j in i) {
                    if (types.find(e => e == i[j])) {
                        const searchType = i[j];
                         newValues[i[j]] = values[searchType] || baseNacionalData[capitalize(searchType)];
                     }
                }
                ...
                newConstraint.push(generateConstraint('ambito', 'equals', acesso));
                newConstraint.push(generateConstraint('ativo', 'equals', true));
                // Chamar a API com as restrições atualizadas e obter a resposta
                const resposta = await callAPI(newConstraint, newValues);
                respostas[acesso] = resposta;
            }

        } catch (error) {
            console.error(`Erro ao obter resposta para o acesso ${acesso}:`, error);
        }
    }
...
router.post('/buscar/:scope/:cru?', async function (req, res) {
    ...
    try {
        ...
        if (reqType == 'multiple') {
            ...
            resultData = await obterRespostasDeTodasAPIs(constraints, values, nacional.Ambito.BaseNacional);
