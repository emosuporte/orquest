...
const { convertObjectToXML } = require('xml2js');
...
router.post('/buscar/:scope1/:scope2/:cru?', async function (req, res) {
    ...
    if (req.params.cru) {
        ...
        const formattedResponse = responseAPIs.map(responseAPI => ({
            api: responseAPI.api._id,
            response: {
                data: convertObjectToXML(responseAPI.res.data), // Converte para XML
                status: responseAPI.res.status,
                contentType: responseAPI.res.headers['content-type'],
            }
        }));

        res.set('Content-Type', 'application/xml'); // Define o tipo de conteúdo como XML
        res.status(200).send(convertObjectToXML(formattedResponse)); // Converte a resposta completa para XML
    }
});
