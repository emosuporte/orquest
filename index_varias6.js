...
const logRequest = (apiURL, method, response, error = null, api) => {
    const log = [
        `consultaUrl: ${apiURL}`,
        ...
        `tempoExecucao: ${response ? `${response.config.responseTimestamp - response.config.requestTimestamp}ms` : ''}`,
    ...
    for (let j = 0; j < scopes.length; j++) {
        ...
        try {
            ...
            for (let i = 0; i < response.data.response.results.length; i++) {
                ...
                logRequest(apiURL, method, resultResponse, null, api);
